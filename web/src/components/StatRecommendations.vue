...
<template>
  <div class="recommended-stats">
    <div v-for="(name, key) in statLabels" :key="key" class="recommended-stat">
      <span>{{ name }}</span><strong>{{ formatTarget(targets[key]) }}</strong>
      <div v-if="editable" class="target-inputs">
        <a-input-number :aria-label="`${name}建议下限`" :value="targets[key]?.min" :min="0" :max="200" :precision="0" placeholder="下限" @change="emit('change', key, 'min', $event)" />
        <span>–</span>
        <a-input-number :aria-label="`${name}建议上限`" :value="targets[key]?.max" :min="0" :max="200" :precision="0" placeholder="上限" @change="emit('change', key, 'max', $event)" />
      </div>
    </div>
  </div>
  <p class="target-caption">配装建议，不是已拥有护甲的实测值。留空表示无要求；支持 0–200，下限与上限相同表示指定目标。随机掉落与最终搭配需自行调整。</p>
</template>
