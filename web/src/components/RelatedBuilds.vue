...
<template>
  <section class="related-builds" aria-label="使用此条目的公开构筑">
    <header><h2>相关构筑</h2><span>{{ matches.length }} 套</span></header>
    <RouterLink v-for="build in matches" :key="build.id" :to="`/builds/${build.id}`"><strong>{{ build.name }} →</strong><p>{{ build.goal }}</p></RouterLink>
    <p v-if="!matches.length">暂时没有引用此条目的公开构筑。</p>
    <RouterLink to="/builds">浏览公开构筑 →</RouterLink>
  </section>
</template>
<style scoped>
