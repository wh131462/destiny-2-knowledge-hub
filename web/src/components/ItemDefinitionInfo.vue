...
<template>
  <div v-if="labels.length || conditions.length || crafting.length || (!compact && item?.tooltipNotifications?.length)" class="definition-info" :class="{ compact }">
    ...
    <template v-else>
      ...
      <section v-if="item?.tooltipNotifications?.length" class="definition-notices" :aria-label="ui('官方情境提示')">
        <strong>{{ ui("官方情境提示") }}</strong>
        <p>{{ ui("定义包含不同情境下的提示，以下文字不表示这些状态同时成立。") }}</p>
        <ul><li v-for="(notice, index) in item.tooltipNotifications" :key="index">{{ localizedField(notice, 'displayString') }}</li></ul>
      </section>
    </template>
  </div>
</template>
...
<style scoped>
.definition-info{min-width:0;margin:.65rem 0;color:var(--text-sub);text-align:left;font-size:.74rem;line-height:1.7;white-space:normal}.definition-tags{display:flex;flex-wrap:wrap;gap:.3rem}.definition-tags>span{padding:.14rem .45rem;border:1px solid var(--line-soft);color:var(--gold-bright);background:var(--bg-card);font-size:.65rem;overflow-wrap:anywhere}.definition-conditions{margin-top:.7rem;padding-top:.6rem;border-top:1px solid var(--line-soft)}.definition-conditions strong{color:var(--gold-bright);font-size:.75rem}.definition-notices{margin-top:.8rem;padding:.72rem 0 .1rem .8rem;border-top:1px solid var(--line-soft);border-left:2px solid var(--gold-dim)}.definition-notices strong{color:var(--gold-bright);font-size:.75rem}.definition-notices ul{color:var(--text-sub)}.definition-info p,.definition-caption{font-size:.7rem;color:var(--text-dim);margin:.35rem 0}.definition-info ul{padding-left:1.15rem;margin:.4rem 0}.definition-info li{overflow-wrap:anywhere}.definition-info small{color:var(--text-dim)}.condition-brief{display:block;margin-top:.35rem;color:var(--text-sub);font-size:.68rem;overflow-wrap:anywhere}.compact{margin:.45rem 0}.definition-info :focus-visible{outline:2px solid var(--gold);outline-offset:3px}
</style>
