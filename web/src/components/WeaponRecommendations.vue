...
<template>
  <dl class="perk-recommendations"><template v-for="column in perkColumns" :key="column.key"><div v-if="recommendation.columns[column.key].length"><dt>{{ column.label }}</dt><dd><template v-for="(name, index) in recommendation.columns[column.key]" :key="name"><span v-if="index"> / </span><RouterLink :to="{ path: '/manifest', query: { type: 'plugs', q: name, filter: 'all' } }">{{ name }} ↗</RouterLink></template></dd></div></template><div v-if="recommendation.unassigned.length"><dt>待核对栏目</dt><dd>{{ recommendation.unassigned.join(' / ') }}</dd></div><div v-if="!selection.perks?.length"><dt>词条建议</dt><dd>未指定；可在编辑副本中补充</dd></div></dl>
</template>
<style scoped>
