...
const filteredGroups = computed(() => allGroups.value.map(group => group.filter(item =>
  ...
  (!query.value || [item.name, item.nameZh, String(item.hash), versionSummary(item), traitText(item), ...setsFor(item).flatMap(set => [set.name, set.nameZh])].join(' ').toLowerCase().includes(query.value))
...
const groupDifferences = group => [...new Set(group.flatMap(versionLabels))].filter(label => !versionLabels(group[0]).includes(label)).join(' / ')
...
function reset() { keyword.value = ''; slotFilter.value = ''; rarityFilter.value = ''; modScope.value = 'slot'; modVersion.value = 'standard' }
watch([keyword, classId, slotFilter, rarityFilter, modScope, modVersion, tab], () => { limit.value = 48 })
...
<template>
  <div class="armor-page">
    <header class="armor-hero">
      <div class="hero-copy"><span class="eyebrow">GUARDIAN FIELD MANUAL <i></i> 06 / ARMOR</span><h1>{{ t('pages.armor.title') }}<span class="title-mark" aria-hidden="true">/</span></h1><p>{{ ui("让每一个部位，都成为构筑的一部分。") }}</p><span class="hero-description">{{ ui("查套装效果、选异域核心、理清属性与模组，再把五个部位组合起来。") }}</span></div>
      <div class="hero-facts"><div><strong>{{ catalog?.sets.length ?? '—' }}</strong><span>{{ ui("官方套装") }}</span></div><div><strong>{{ catalog ? exoticCount : '—' }}</strong><span>{{ ui("异域款式") }}</span></div><div><strong>05</strong><span>{{ ui("护甲部位") }}</span></div></div>
    </header>
    <div class="snapshot-line"><span><i></i> BUNGIE MANIFEST</span><span v-if="catalog">{{ ui("快照") }} {{ catalog.syncedAt?.slice(0, 10) }} {{ catalog.items.length.toLocaleString() }} {{ ui("个防具定义，含同名版本") }}</span><span v-else>{{ ui("正在连接防具目录") }}</span></div>
    <nav class="armor-tabs" :aria-label="ui(&quot;防具栏目&quot;)"><button v-for="item in tabs" :key="item.id" type="button" :aria-pressed="tab === item.id" @click="tab = item.id"><span>{{ ui(item.label) }}</span><small>{{ item.en }}</small></button></nav>
    <div v-if="tab !== 'guide'" class="armor-toolbar">
      <div v-if="tab !== 'mods'" class="class-picker" role="group" :aria-label="ui(&quot;职业筛选&quot;)"><button v-for="c in armorClasses" :key="c.id" type="button" :aria-pressed="classId === c.id" @click="classId = c.id">{{ ui(c.name) }}</button></div>
      <a-input v-model:value="keyword" allow-clear :placeholder="tab === 'sets' ? ui(&quot;搜索套装名称或效果，如：手雷、治疗…&quot;) : tab === 'mods' ? ui(&quot;搜索模组名称或效果…&quot;) : ui(&quot;搜索防具、异域特性或 Hash…&quot;)" :aria-label="ui(&quot;搜索防具目录&quot;)" class="armor-search" />
      <button class="reset-button" type="button" @click="reset">{{ ui("重置") }}</button>
    </div>
    ...
    <template v-else-if="catalog">
      ...
      <template v-else-if="tab !== 'sets'">
        <div v-if="tab !== 'mods' || modScope === 'slot'" class="slot-picker" role="group" :aria-label="ui(&quot;部位筛选&quot;)"><button type="button" :aria-pressed="!slotFilter" @click="slotFilter = ''">{{ ui("全部部位") }}</button><button v-for="slot in armorSlots" :key="slot.id" type="button" :aria-pressed="slotFilter === slot.id" @click="slotFilter = slot.id">{{ localized(slot) }}<small v-if="slot.id === 'classItem' && tab !== 'mods'"> / {{ classItemName }}</small></button></div>
        <template v-if="tab === 'all' || tab === 'exotics'">
          <div class="archive-heading"><div><h2>{{ tab === 'exotics' ? ui("以一件异域，定义你的打法") : ui("你的防具资料库") }}</h2><p>{{ tab === 'exotics' ? ui("每次最多装备一件异域护甲；可与一件异域武器同时装备。") : ui("按职业和部位查阅官方装备；同名且套装归属相同的版本合并展示。") }}</p></div><label v-if="tab === 'all'" class="archive-filter" for="armor-rarity"><span class="sr-only">{{ ui("稀有度筛选") }}</span><a-select id="armor-rarity" v-model:value="rarityFilter" :aria-label="ui(&quot;稀有度筛选&quot;)"><a-select-option value="">{{ ui("全部稀有度") }}</a-select-option><a-select-option value="exotic">{{ ui("异域") }}</a-select-option><a-select-option value="legendary">{{ ui("传说") }}</a-select-option></a-select></label></div>
          <p class="result-count" role="status">{{ filteredGroups.length }} {{ ui("款匹配 已展示") }} {{ visibleGroups.length }} {{ ui("款") }}</p>
          <div class="armor-grid"><button v-for="group in visibleGroups" :key="group[0].hash" type="button" class="armor-tile" :class="{ exotic: isExotic(group[0]) }" @click="openArmor(group[0], group)"><div class="armor-icon"><img v-if="icon(group[0])" :src="icon(group[0])" alt="" loading="lazy" /><span v-else>◇</span></div><div class="armor-tile-copy"><span class="tile-meta">{{ ui(rarityName(group[0])) }} / {{ ui(slotName(group[0])) }}</span><h3>{{ armorName(group[0]) }}</h3><small class="english-name">{{ group[0].name }}</small><p v-if="primaryTrait(group[0])" class="trait-line">{{ primaryTrait(group[0]) }}</p><p v-else>{{ setsFor(group[0]).map(armorName).join('、') || ui("未登记套装加成") }}</p><ItemDefinitionInfo :item="group[0]" compact /><small v-if="groupDifferences(group)" class="group-differences">{{ ui("组内还含：") }}{{ groupDifferences(group) }}</small><span class="tile-footer">{{ group.length }} {{ ui("个版本") }} <span>{{ ui("查看详情 ↗") }}</span></span></div></button></div>
          <div v-if="!filteredGroups.length" class="empty-state"><h3>{{ ui("没有匹配的防具") }}</h3><p>{{ ui("试试其他职业、部位，或缩短关键词。") }}</p><button type="button" class="outline-button" @click="reset">{{ ui("清空筛选") }}</button></div>
          <button v-if="filteredGroups.length > limit" type="button" class="load-more" @click="limit += 48">{{ ui("加载更多 还有") }} {{ filteredGroups.length - limit }} {{ ui("款") }}</button>
        </template>
        <template v-else-if="tab === 'mods'">
          <div class="archive-heading"><div><h2>{{ ui("为每个部位找到合适的模组") }}</h2><p>{{ ui("查看官方效果与能量消耗；特殊模组的可用性还需符合装备插槽和活动条件。") }}</p></div><label class="archive-filter" for="armor-mod-scope"><span class="sr-only">{{ ui("模组范围") }}</span><a-select id="armor-mod-scope" v-model:value="modScope" :aria-label="ui(&quot;模组范围&quot;)"><a-select-option value="slot">{{ ui("按部位模组") }}</a-select-option><a-select-option value="special">{{ ui("通用与活动模组") }}</a-select-option><a-select-option value="all">{{ ui("所有模组") }}</a-select-option></a-select></label></div>
          <label class="archive-filter mod-version-filter" for="armor-mod-version">{{ ui("模组版本") }}<a-select id="armor-mod-version" v-model:value="modVersion" :aria-label="ui(&quot;模组版本&quot;)" style="min-width: 180px"><a-select-option value="standard">{{ ui("普通定义（不含神器限定）") }}</a-select-option><a-select-option value="artifact">{{ ui("需神器解锁的版本") }}</a-select-option><a-select-option value="all">{{ ui("全部定义（含历史与占位）") }}</a-select-option></a-select></label>
          <p class="mod-note">{{ ui("同一件护甲的模组消耗合计不能超过其能量容量。模组图鉴包含历史定义，不表示当前掉落装备都能安装。") }}</p>
          <p class="result-count" role="status">{{ filteredMods.length }} {{ ui("个匹配 显示") }} {{ Math.min(limit, filteredMods.length) }} {{ ui("个") }}</p>
          <div class="mod-grid"><article v-for="mod in filteredMods.slice(0, limit)" :key="mod.hash" class="mod-card"><header><img v-if="icon(mod)" :src="icon(mod)" alt="" loading="lazy" /><div><h3>{{ armorName(mod) }}</h3><small>{{ mod.name }}</small></div><span class="energy-cost" :aria-label="mod.energyCost == null ? ui(&quot;能量消耗未登记&quot;) : ui(&quot;能量消耗 {0}&quot;, [mod.energyCost])">{{ mod.energyCost ?? '—' }}<small>{{ ui("能量") }}</small></span></header><ItemDefinitionInfo :item="mod" compact /><p>{{ description(mod) || ui("该定义未提供效果说明。") }}</p><small v-if="mod.differingPerkNames?.length" class="mod-definition-note">{{ ui("关联效果：") }}{{ mod.differingPerkNames.join('、') }}{{ ui("（与条目名称不同）") }}</small><small class="mod-definition-note">{{ ui("定义") }} {{ mod.hash }}</small><footer><span>{{ armorSlots.find(slot => slot.id === mod.slot)?.name || ui("通用 / 活动专属") }}</span><EntityLink :item="mod" kind="mods" :label="ui(&quot;模组详情&quot;)" /></footer></article></div>
          <div v-if="!filteredMods.length" class="empty-state"><h3>{{ ui("未找到匹配模组") }}</h3><button type="button" class="outline-button" @click="reset">{{ ui("清空筛选") }}</button></div>
          <button v-if="filteredMods.length > limit" type="button" class="load-more" @click="limit += 48">{{ ui("加载更多 还有") }} {{ filteredMods.length - limit }} {{ ui("个") }}</button>
        </template>
      </template>
    </template>
    ...
    <a-modal :open="Boolean(selectedArmor)" :title="selectedArmor ? armorName(selectedArmor) : ''" :footer="null" :width="760" wrap-class-name="armor-modal" @cancel="selectedArmor = null">
      <div v-if="selectedArmor" class="armor-detail">
        <header class="detail-heading"><img v-if="icon(selectedArmor)" :src="icon(selectedArmor)" alt="" /><div><span :class="{ 'exotic-label': isExotic(selectedArmor) }">{{ ui(rarityName(selectedArmor)) }} {{ ui(className(selectedArmor)) }} {{ ui(slotName(selectedArmor)) }}</span><h2>{{ armorName(selectedArmor) }}</h2><p>{{ selectedArmor.name }}</p><small>Hash {{ selectedArmor.hash }}</small></div></header>
        <label v-if="selectedVersions.length > 1" class="version-select" for="armor-version">{{ ui("装备版本") }}<a-select id="armor-version" popup-class-name="armor-version-options" :aria-label="ui(&quot;装备版本&quot;)" :value="selectedArmor.hash" @change="selectedArmor = selectedVersions.find(item => item.hash === $event)"><a-select-option v-for="(version, index) in selectedVersions" :key="version.hash" :value="version.hash">{{ versionSummary(version) || ui("未登记功能差异") }} {{ ui("· 定义") }} {{ version.hash }}</a-select-option></a-select></label>
        <ItemDefinitionInfo :item="selectedArmor" /><p v-if="selectedVersions.length > 1" class="detail-note">{{ ui("选项展示该定义的插槽特征；同名版本不代表相同功能，锁定槽需在游戏中解锁。") }}</p>
        <section v-if="selectedTraits.length" class="detail-section"><h3>{{ selectedArmor.armorSlot === 'classItem' && isExotic(selectedArmor) ? ui("双列异域特性") : selectedTraits.some(pool => pool.options.length > 1) ? ui("异域特性与可选词条") : ui("固有特性") }}</h3><p v-if="selectedArmor.armorSlot === 'classItem' && isExotic(selectedArmor)" class="detail-note">{{ ui("异域职业装备拥有两列特性。当前快照可能仅提供插槽默认示例；下列说明不代表固定掉落组合，完整候选池未返回时无法推导其他组合。") }}</p><div v-for="(pool, index) in selectedTraits" :key="pool.socketIndex"><p v-if="pool.options.length > 1" class="detail-note">{{ ui("第") }} {{ index + 1 }} {{ ui("列") }} {{ pool.options.length }} {{ ui("个候选，实际掉落从该列选择特性，以下并非同时生效。") }}</p><p v-else-if="selectedArmor.armorSlot === 'classItem' && pool.definitionOnly" class="detail-note">{{ ui("第") }} {{ index + 1 }} {{ ui("列 定义默认示例，完整候选池未提供") }}</p><article v-for="trait in pool.options" :key="trait.hash" class="trait-detail"><img v-if="icon(trait)" :src="icon(trait)" alt="" loading="lazy" /><div><h4>{{ armorName(trait) }}</h4><p>{{ description(trait) || ui("该特性暂无说明。") }}</p></div></article></div></section>
        <p v-else-if="isExotic(selectedArmor)" class="detail-note">{{ ui("该版本的固有特性尚未解析，可切换版本或打开百科查阅。") }}</p>
        <section v-if="setsFor(selectedArmor).length" class="detail-section"><h3>{{ ui("所属套装") }}</h3><article v-for="set in setsFor(selectedArmor)" :key="set.hash"><h4>{{ armorName(set) }}</h4><div v-for="perk in set.perks" :key="perk.sandboxPerkHash" class="detail-set-bonus"><strong>{{ perk.requiredSetCount }} {{ ui("件") }} {{ armorName(perk) }}</strong><p>{{ description(perk) }}</p></div></article></section>
        <p v-if="description(selectedArmor)" class="armor-flavor">{{ description(selectedArmor) }}</p>
        <section class="detail-section"><h3>{{ ui("属性与装备实例") }}</h3><p class="detail-note">{{ ui("此处收录装备定义，随机掉落的六维属性、Tier、能量等级与大师状态由具体实例决定。请在游戏内检查实物数值后用于配装。") }}</p></section>
        <section class="detail-section"><h3>{{ ui("获取线索") }}</h3><template v-if="selectedSources.length"><div v-for="(source, index) in selectedSources" :key="index" class="source-row"><span>{{ source.type }}</span><strong>{{ source.name }}</strong></div><p class="detail-note">{{ ui("定义关联不保证当前轮换可用，也不代表具体遭遇战的掉落概率。") }}</p></template><p v-else class="detail-note">{{ ui("当前定义没有直接关联的活动奖励或商人记录，无法据此确定当前获取路径。") }}</p></section>
        <EntityLink :item="selectedArmor" kind="equipment" :label="ui(&quot;打开独立百科条目&quot;)" /><SourceProvenance :item="selectedArmor" :snapshot="catalog || {}" official compact />
      </div>
    </a-modal>
  </div>
</template>
...
@media(max-width:560px){.armor-grid,.mod-grid{grid-template-columns:1fr}.armor-hero h1{font-size:2.15rem}.eyebrow{font-size:.48rem;gap:8px}.hero-copy>p{font-size:.9rem}.detail-heading{gap:13px}.detail-heading>img{width:62px;height:62px}.detail-heading h2{font-size:1.2rem}.version-select{align-items:start;flex-direction:column;gap:8px}.version-select .ant-select{flex:auto}.source-row{flex-direction:column;gap:3px}}
@media(prefers-reduced-motion:reduce){.armor-tile{transition:none}}
</style>

<style scoped>
.mod-version-filter{display:grid;gap:.35rem;max-width:320px;margin:1rem 0}:global(.armor-version-options .ant-select-item-option-content){white-space:normal;line-height:1.6;overflow-wrap:anywhere}.group-differences{display:block;white-space:normal;line-height:1.7;color:var(--text-sub);overflow-wrap:anywhere}.version-select :deep(.ant-select-selection-item){white-space:normal;line-height:1.6}.version-select :deep(.ant-select-selector){height:auto!important;min-height:36px;padding-block:6px!important}.archive-filter{max-width:100%}.archive-filter :deep(.ant-select){max-width:100%}
</style>
