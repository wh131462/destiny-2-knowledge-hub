    ...
    <template v-else-if="catalog">
      ...
      <template v-else-if="tab !== 'sets'">
        ...
        <template v-if="tab === 'all' || tab === 'exotics'">
          <div class="archive-heading"><div><h2>{{ tab === 'exotics' ? '以一件异域，定义你的打法' : '你的防具资料库' }}</h2><p>{{ tab === 'exotics' ? '每次最多装备一件异域护甲；可与一件异域武器同时装备。' : '按职业和部位查阅官方装备；同名且套装归属相同的版本合并展示。' }}</p></div><label v-if="tab === 'all'" class="archive-filter" for="armor-rarity"><span class="sr-only">稀有度筛选</span><a-select id="armor-rarity" v-model:value="rarityFilter" aria-label="稀有度筛选"><a-select-option value="">全部稀有度</a-select-option><a-select-option value="exotic">异域</a-select-option><a-select-option value="legendary">传说</a-select-option></a-select></label></div>
        ...
        <template v-else-if="tab === 'mods'">
          <div class="archive-heading"><div><h2>为每个部位找到合适的模组</h2><p>查看官方效果与能量消耗；特殊模组的可用性还需符合装备插槽和活动条件。</p></div><label class="archive-filter" for="armor-mod-scope"><span class="sr-only">模组范围</span><a-select id="armor-mod-scope" v-model:value="modScope" aria-label="模组范围"><a-select-option value="slot">按部位模组</a-select-option><a-select-option value="special">通用与活动模组</a-select-option><a-select-option value="all">所有模组</a-select-option></a-select></label></div>
