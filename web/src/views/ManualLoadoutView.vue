...
<template>
  <div class="manual-page build-flow">
    ...
    <details class="snapshot"><summary>{{ ui("装备数据与配装说明") }}</summary><p>{{ ui("装备快照：") }}{{ snapshot.syncedAt?.slice(0, 10) || ui("加载中") }} {{ snapshot.manifestVersion || '—' }}{{ ui("。这是一份推荐清单，不读取账号或判断是否拥有装备。") }}</p></details>
    ...
    <article class="loadout-sheet" :aria-busy="status === 'loading'">
      ...
      <div class="top-grid">
        <section id="edit-talents" data-build-section tabindex="-1" class="panel talents"><h2><b>01</b> {{ ui("天赋与技能") }}</h2><p class="muted">{{ ui("来自当前 Manifest 子职业的完整可选插槽；不按推荐构筑或账号解锁状态缩减。") }}</p>
          ...
          <div v-if="intrinsicAbilities.length" class="intrinsic-abilities"><details v-for="item in intrinsicAbilities" :key="item.id"><summary><img v-if="visualIcon(item)" :src="visualIcon(item)" alt="" /><span>{{ localized(item) }}<small>{{ item.kind === 'transcendenceGrenade' ? ui("超越期间替换手雷 固定能力") : ui("棱镜固定能力 点击查看") }}</small></span></summary><p>{{ visualDescription(item) }}</p></details></div>
      ...
      <footer id="edit-checks" data-build-section tabindex="-1" class="sheet-footer"><details v-if="Object.values(draft.mods).flat().some(row => conditionSummary(mod(row.manifestHash)))"><summary>{{ ui("已选模组的使用条件（账号状态未核对）") }}</summary><div v-for="(row, index) in Object.values(draft.mods).flat()" :key="index"><strong>{{ label(mod(row.manifestHash)) }}</strong><ItemDefinitionInfo :item="mod(row.manifestHash)" /></div></details><p>{{ ui("推荐草稿可随时保存与导出。空白项表示未指定，不代表缺少装备。") }}</p><p v-if="draft.manifestVersion && draft.manifestVersion !== snapshot.manifestVersion">{{ ui("导入来源版本与本地快照不同，保留原推荐，请重新核对。") }}</p><details v-if="issues.length" open><summary>{{ ui("配置提示") }}</summary><ul><li v-for="issue in issues" :key="issue">{{ uiMessage(issue) }}</li></ul></details></footer>
    </article>
...
.snapshot { font-size: .75rem; }.snapshot summary { cursor: pointer; color: var(--text-sub); }.snapshot p { padding: .6rem 0; overflow-wrap: anywhere; }
