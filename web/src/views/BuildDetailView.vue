...
<template>
  <div v-if="build" class="detail-page">
    <router-link to="/builds" class="back">← 返回构筑方案</router-link>
    <header class="detail-head"><div><p>{{ build.subclassId.includes('prismatic') ? 'PRISMATIC' : 'SUBCLASS' }} / {{ build.difficulty }}</p><h1>{{ build.name }}</h1><span>{{ build.goal }}</span></div><div class="verify"><ConfidenceBadge :level="build.confidence" /><strong>{{ validation.valid ? '编辑推荐' : '存在待核对项' }}</strong><small>核验于 {{ build.verifiedAt }}</small><small class="manifest-hash">Manifest {{ build.manifestVersion }}</small></div></header>

    <p><router-link :to="`/manual-loadout?build=${build.id}`">以此方案创建副本（含词条与备注） ↗</router-link></p>
    <section class="quick-config"><div v-for="abilityId in [build.abilities.superId, build.abilities.classAbilityId, build.abilities.meleeId, build.abilities.grenadeId]" :key="abilityId"><span>{{ abilityById[abilityId]?.kind === 'super' ? '超能力' : abilityById[abilityId]?.kind === 'classAbility' ? '职业技能' : abilityById[abilityId]?.kind === 'melee' ? '近战' : '手雷' }}</span><div class="quick-icon"><img v-if="icon(abilityById[abilityId])" :src="icon(abilityById[abilityId])" :alt="abilityName(abilityId)" /></div><b><EntityLink :item="abilityById[abilityId]" /></b></div><div><span>护甲套装</span><div class="quick-icon armor-mark"><b>{{ armorSetName(build.armorSetId).slice(0, 1) }}</b></div><b><EntityLink :item="armorSetById[build.armorSetId]" /></b></div></section>

    <div class="two-col">
      <section class="content-section"><div class="section-no">01</div><div><h2>完整子职业配置</h2><h3>星相</h3><div class="tokens"><span v-for="id in build.abilities.aspectIds" :key="id"><img v-if="icon(aspectById[id])" :src="icon(aspectById[id])" :alt="aspectById[id].name" /><strong><EntityLink :item="aspectById[id]" /></strong><small>{{ aspectById[id].en }}</small></span></div><h3>{{ build.abilities.facetIds?.length ? '棱镜特性' : '元素碎片' }}</h3><div v-if="build.abilities.facetIds?.length" class="tokens facets"><span v-for="id in build.abilities.facetIds" :key="id"><strong><EntityLink :item="facetById[id]" /></strong><small>{{ facetById[id].description }}</small></span></div><div v-else class="tokens facets"><span v-for="id in build.abilities.fragmentIds" :key="id"><strong><EntityLink :item="fragmentById[id]" /></strong><small>{{ fragmentById[id].description }}</small></span></div></div></section>
      <section class="content-section"><div class="section-no">02</div><div><h2>六维属性建议</h2><StatRecommendations :targets="build.targetStats" /></div></section>
    </div>

    <section class="content-section full"><div class="section-no">03</div><div><h2>武器和职责</h2><div class="weapon-table"><article v-for="item in build.weapons" :key="item.itemId"><div class="weapon-art"><img v-if="icon(gearById[item.itemId])" :src="icon(gearById[item.itemId])" :alt="gearName(item.itemId)" /><span v-else>{{ gearName(item.itemId).slice(0, 1) }}</span></div><header><strong><EntityLink :item="gearById[item.itemId]" /></strong><span>{{ gearById[item.itemId].slot }} / {{ gearById[item.itemId].ammo }}</span></header><WeaponRecommendations :selection="item" :equipment="equipmentItems" /><p>{{ item.purpose }}</p></article></div></div></section>

    <section class="content-section full"><div class="section-no">04</div><div><h2>五个护甲部位模组</h2><div class="mod-slots"><article v-for="(mods,slot) in build.armorMods" :key="slot"><header><strong>{{ slotNames[slot] }}</strong><span>{{ mods.reduce((sum,id)=>sum+(modById[id]?.cost||0),0) }}/10</span></header><ul><li v-for="id in mods" :key="id"><b><EntityLink :item="modById[id]" /></b><span>{{ modById[id]?.cost }} 能量</span><p>{{ modById[id]?.effect || `+${modById[id]?.value} ${modById[id]?.stat}` }}</p><small v-if="officialModFor(id)?.primaryHash" class="manifest-hash">官方 Hash {{ officialModFor(id).primaryHash }}<span v-if="!officialModFor(id).consistent">：待复核</span></small></li></ul></article></div><p class="mod-note">六维显示的是目标，不从固定基础值模拟掉落。模组按一个通用属性位、三个部位位和满升级 10 能量规划；具体护甲版本请在编辑副本中核对兼容插槽。</p></div></section>
    ...
    <section class="content-section full"><div class="section-no">06</div><div><h2>机制和反冠军覆盖</h2><div class="mechanic-grid"><article v-for="id in build.mechanicIds" :key="id"><strong><EntityLink :item="mechanicById[id]" /></strong><span>{{ mechanicById[id].en }}</span><p>{{ mechanicById[id].values.effect || (mechanicById[id].values.standardPvePercent ? `标准 PvE 削弱 ${mechanicById[id].values.standardPvePercent}%` : '参与该构筑的技能链') }}</p></article></div><div class="champions"><div v-for="type in ['barrier','overload','unstoppable']" :key="type"><span>{{ type }}</span><p v-if="build.championCoverage[type].length">{{ build.championCoverage[type].join('；') }}</p><p v-else class="gap">缺口：进入对应活动前必须更换武器或由队友覆盖</p></div></div></div></section>

    <section class="content-section full acquisition-section"><div class="section-no">07</div><div class="acquisition-content"><div class="acquisition-head"><div><h2>获取与刷装参考</h2><p>获取路线为编辑资料，随机掉落和轮换请在游戏内核对；可在推荐副本中记录刷装备注。</p></div></div><div class="acquisition-list"><article v-for="item in acquisition" :key="item.itemId"><header><span>{{ item.order }}</span><strong>{{ item.name }}</strong></header><div><b>{{ item.pathName }}</b><small>{{ item.deterministic ? '确定获取路径' : '随机掉落 / 需要刷取' }}：{{ item.access }}，编辑整理步骤</small><ol><li v-for="step in item.steps" :key="step">{{ step }}</li></ol><div v-if="officialFor(item.itemId)?.vendorSources?.length || officialFor(item.itemId)?.activitySources?.length" class="official-sources"><small>Manifest 官方来源证据</small><span v-for="source in [...(officialFor(item.itemId)?.vendorSources || []).slice(0, 2), ...(officialFor(item.itemId)?.activitySources || []).slice(0, 2)]" :key="`${source.vendorHash || source.activityHash}-${source.itemHash}`">{{ source.vendorName || source.activityName }}</span></div><small v-if="officialFor(item.itemId)?.manifestHashes?.length" class="manifest-hash">Hash {{ officialFor(item.itemId).primaryHash }}：{{ officialFor(item.itemId).manifestHashes.length }} 个版本</small></div></article></div></div></section>
    ...
    <section class="limits"><h2>已知限制与快照校验</h2><ul><li v-for="item in validation.errors" :key="`${item.code}-${item.path}`">{{ item.message }}</li><li v-for="item in build.limitations" :key="item">{{ item }}</li><li v-for="item in validation.warnings" :key="item.message">{{ item.message }}</li></ul></section>
  </div>
  <div v-else class="empty"><h1>未找到构筑</h1><router-link to="/builds">返回构筑方案</router-link></div>
</template>
