  ...
  <div class="entry-page">
    ...
    <template v-else-if="item">
      ...
      <div class="entry-layout"><div class="entry-content">
        ...
        <section v-if="item.perkSockets?.length" class="entry-section"><h2><span aria-hidden="true"></span> {{ ui("词条候选") }}</h2><div class="socket-actions"><StableDisclosure v-for="socket in item.perkSockets" :key="socket.socketIndex" :title="`${ui('插槽')} ${socket.socketIndex + 1}`" :width="760" trigger-class="socket-trigger"><template #trigger><span>{{ ui("插槽") }} {{ socket.socketIndex + 1 }}</span><small>{{ socket.plugItemHashes.length }} {{ ui("个候选") }}</small></template><div class="entry-links socket-candidates"><div v-for="perk in candidates(socket)" :key="perk.hash"><EntityLink :item="perk" kind="plugs" /><ItemDefinitionInfo :item="perk" /></div></div></StableDisclosure></div></section>
