...
<template>
  <div>
    ...
    <section><div class="section-title"><h2>站内百科与公开构筑</h2><span class="en">EDITORIAL COLLECTION</span></div><p class="coverage-note">以下是站内使用的技能逻辑条目、机制解读与公开构筑数量，不是全游戏实体总量。这里统计站内编辑目录；社区投稿由独立公开快照提供，配置检查不等于实机验证。</p><div class="metrics"><div v-for="item in metrics" :key="item[0]"><strong>{{ item[1] }}</strong><span>{{ item[0] }}</span></div></div></section>
    ...
    <section class="manifest-block"><h2>子职业技能池覆盖</h2><p class="coverage-note">按当前快照的可选插槽逐项生成；棱镜固定超越能力另行展示，不混入普通手雷池。</p><div class="skill-coverage"><table><thead><tr><th>子职业</th><th>超能</th><th>职业</th><th>跳跃</th><th>近战</th><th>手雷</th><th>星相</th><th>碎片</th></tr></thead><tbody><tr v-for="sub in subclasses" :key="sub.id"><th>{{ sub.name }}</th><td>{{ sub.superIds.length }}</td><td>{{ sub.classAbilityIds.length }}</td><td>{{ sub.movementIds.length }}</td><td>{{ sub.meleeIds.length }}</td><td>{{ sub.grenadeIds.length }}</td><td>{{ sub.aspectIds.length }}</td><td>{{ sub.fragmentIds.length || sub.facetIds.length }}</td></tr></tbody></table></div></section>
