...
import CatalogCoverage from '@/components/CatalogCoverage.vue'
...
const metrics = [
  ...
  ['装备实体', gearItems.length], ['护甲套装', armorSets.length], ['异域职业物品特性', exoticClassItemTraits.length], ['模组', armorMods.length], ['活动场景', activitiesV2.length], ['公开构筑', curatedBuilds.length]
]

const gaps = [
  '普通与棱镜技能池从官方可装备子职业及其 PlugSet 生成，不再以推荐构筑清单限制选项。',
  'Bungie Manifest 已接入组件级快照，网站会展示最近同步版本与条目数。',
  '超能、职业技能、跳跃、近战、手雷、星相、碎片及固定超越能力均保留对应子职业 Hash；不读取账号解锁状态。',
  '公开武器与护甲定义按 Hash 对账；逐补丁沙盒实测数值、账号解锁与实时轮换不能从静态 Manifest 推断。',
...
<template>
  <div>
    ...
    <CatalogCoverage />
    <section class="versions"><div><span>{{ ui("数据快照") }}</span><strong>{{ DATA_VERSION }}</strong></div><div><span>{{ ui("Manifest 版本") }}</span><strong>{{ MANIFEST_VERSION }}</strong></div><div><span>{{ ui("规则引擎 / 构筑") }}</span><strong>{{ RULES_VERSION }}：{{ curatedBuilds.length }}</strong></div></section>
    <section><div class="section-title"><h2>{{ ui("站内百科与公开构筑") }}</h2><span class="en">EDITORIAL COLLECTION</span></div><p class="coverage-note">{{ ui("以下是站内使用的技能逻辑条目、机制解读与公开构筑数量，不是全游戏实体总量。这里统计站内编辑目录；社区投稿由独立公开快照提供，配置检查不等于实机验证。") }}</p><div class="metrics"><div v-for="item in metrics" :key="item[0]"><strong>{{ item[1] }}</strong><span>{{ ui(item[0]) }}</span></div></div></section>
    <section><div class="section-title"><h2>{{ ui("已知缺口") }}</h2><span class="en">KNOWN GAPS</span></div><ol class="gaps"><li v-for="item in gaps" :key="item">{{ item }}</li></ol></section>
    <section><div class="section-title"><h2>{{ ui("来源登记") }}</h2><span class="en">SOURCES</span></div><div class="sources"><article v-for="source in sources" :key="source.id"><ConfidenceBadge :level="source.level" /><div><h3>{{ source.title }}</h3><p>{{ source.note }}</p><small>{{ source.publisher }}{{ ui("：核验") }} {{ source.checkedAt }}</small></div><a v-if="source.url" :href="source.url" target="_blank" rel="noreferrer">{{ ui("打开 ↗") }}</a></article></div></section>
    <section v-if="manifestStatus" class="manifest-block"><div class="section-title"><h2>{{ ui("Manifest 快照") }}</h2><span class="en">OFFICIAL SYNC</span></div><div class="manifest-meta"><div><span>{{ ui("版本") }}</span><strong>{{ manifestStatus.manifestVersion }}</strong></div><div><span>{{ ui("同步时间") }}</span><strong>{{ manifestStatus.syncedAt }}</strong></div><div><span>{{ ui("语言") }}</span><strong>{{ manifestStatus.locale }}</strong></div></div><div class="manifest-components"><span v-for="item in manifestStatus.components" :key="item.component">{{ item.component }}：{{ item.count }}</span></div></section>
    <section v-if="normalized.equipment || normalized.mods" class="manifest-block"><div class="section-title"><h2>{{ ui("标准化与映射覆盖") }}</h2><span class="en">NORMALIZATION</span></div><div class="normalization-grid"><div><strong>{{ normalized.equipment?.count?.toLocaleString?.() || '—' }}</strong><span>{{ ui("可分类武器 / 护甲") }}</span></div><div><strong>{{ normalized.abilities?.count?.toLocaleString?.() || '—' }}</strong><span>{{ ui("官方技能定义") }}</span></div><div><strong>{{ normalized.activities?.count?.toLocaleString?.() || '—' }}</strong><span>{{ ui("官方活动定义") }}</span></div><div><strong>{{ normalized.mods?.count?.toLocaleString?.() || '—' }}</strong><span>{{ ui("可用装备模组") }}</span></div><div><strong>{{ normalized.modLinks ? `${normalized.modLinks.matched}/${normalized.modLinks.count}` : '—' }}</strong><span>{{ ui("站内模组官方匹配") }}</span></div><div><strong>{{ normalized.modLinks?.consistent ?? '—' }}</strong><span>{{ ui("槽位 / 能量一致") }}</span></div><div><strong>{{ normalized.vendors?.count?.toLocaleString?.() || '—' }}</strong><span>{{ ui("供应商库存条目") }}</span></div><div><strong>{{ normalized.activityRewards?.count?.toLocaleString?.() || '—' }}</strong><span>{{ ui("活动奖励关系") }}</span></div><div><strong>{{ normalized.itemSets?.count?.toLocaleString?.() || '—' }}</strong><span>{{ ui("官方装备套装") }}</span></div><div><strong>{{ normalized.links ? `${normalized.links.matched}/${normalized.links.count}` : '—' }}</strong><span>{{ ui("站内实体 Manifest 映射") }}</span></div><div><strong>{{ normalized.drops?.withVendorSource?.toLocaleString?.() || '0' }}</strong><span>{{ ui("具有供应商来源的装备") }}</span></div><div><strong>{{ normalized.drops?.withActivityReward?.toLocaleString?.() || '0' }}</strong><span>{{ ui("具有活动奖励来源的装备") }}</span></div><div><strong>{{ normalized.drops ? `${normalized.drops.withRewardMapping}/${normalized.drops.count}` : '—' }}</strong><span>{{ ui("RewardMapping 关联") }}</span></div></div><p class="coverage-note">{{ ui("装备分类、官方技能与活动定义、武器 Socket 词条池、模组能量、Sandbox Perk、供应商库存、活动奖励和官方装备套装已从官方定义生成。RewardMappingDefinition 仍没有可用关联，因此无法从它推导更细的遭遇战掉落条件。") }}</p><p v-if="modLinkGaps" class="coverage-note warning">{{ modLinkGaps }} {{ ui("个站内模组在当前 Manifest 中没有可确认的同名实体，保留编辑基线并标记为待复核。") }}</p><p v-if="unmatched.length" class="coverage-note warning">{{ ui("未匹配实体：") }}{{ unmatched.join('、') }}</p></section>
    <section class="manifest-block"><h2>{{ ui("子职业技能池覆盖") }}</h2><p class="coverage-note">{{ ui("按当前快照的可选插槽逐项生成；棱镜固定超越能力另行展示，不混入普通手雷池。") }}</p><div class="skill-coverage"><table><thead><tr><th>{{ ui("子职业") }}</th><th>{{ ui("超能") }}</th><th>{{ ui("职业") }}</th><th>{{ ui("跳跃") }}</th><th>{{ ui("近战") }}</th><th>{{ ui("手雷") }}</th><th>{{ ui("星相") }}</th><th>{{ ui("碎片") }}</th></tr></thead><tbody><tr v-for="sub in subclasses" :key="sub.id"><th>{{ sub.name }}</th><td>{{ sub.superIds.length }}</td><td>{{ sub.classAbilityIds.length }}</td><td>{{ sub.movementIds.length }}</td><td>{{ sub.meleeIds.length }}</td><td>{{ sub.grenadeIds.length }}</td><td>{{ sub.aspectIds.length }}</td><td>{{ sub.fragmentIds.length || sub.facetIds.length }}</td></tr></tbody></table></div></section>
    <div class="policy">{{ dataPolicy.disclaimer }}</div>
  </div>
</template>

<style scoped>
.skill-coverage{overflow-x:auto;margin-top:1rem}.skill-coverage table{width:100%;border-collapse:collapse;font-size:.78rem;min-width:530px}.skill-coverage th,.skill-coverage td{padding:.7rem;text-align:left;border-bottom:1px solid var(--line-soft)}.skill-coverage thead{color:var(--gold-dim)}.skill-coverage td{color:var(--text-sub);font-variant-numeric:tabular-nums}
