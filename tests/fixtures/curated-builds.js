...
// Test-only fixtures for exercising validation and recommendation rules.
// These records are not published by the application and carry no gameplay claim.
...
const curatedBuildsBase = [
  {
    ...
    abilities: {
      ...
      facetIds: ['facet-purpose', 'facet-protection', 'facet-balance', 'facet-ruin']
    },
    ...
    armorMods: armor(
      ['hands-on', 'heavy-ammo-finder', 'stat-health'],
      ['heavy-handed', 'impact-induction', 'stat-grenade'],
      ['concussive-dampener', 'melee-damage-resistance', 'stat-health'],
      ['recuperation', 'void-weapon-surge', 'stat-grenade'],
      ['powerful-attraction', 'reaper', 'bomber']
    ),
    baseStats: { weapons: 20, health: 80, class: 30, grenade: 80, super: 30, melee: 40 },
    targetStats: { health: 100, grenade: 100 },
    mechanicIds: ['scorch', 'ignite', 'healing', 'damage-resistance', 'transcendence', 'suspend'],
    championCoverage: { unstoppable: ['点燃', '束缚手雷悬浮'], barrier: [], overload: [] },
    rotation: [
      '先用束缚手雷控制危险目标；需要保留反不可阻挡手段时不要随意消耗。',
      '滑铲后连续触发奉献上挑与砸地；优先在敌群或精英旁引爆点燃。',
      '拾取近战击杀生成的能量球，以休养生息回血并触发使命之面。',
      '光暗能量充满后进入超越，利用快速技能回复继续奉献循环。',
      '边缘交通按已推荐的充盈刺客与动荡弹药使用；当前词条没有诱导推销，不应按该增伤循环安排输出。'
    ],
  ...
  },
  {
    ...
    abilities: {
      ...
      facetIds: ['facet-purpose', 'facet-protection', 'facet-blessing', 'facet-ruin']
    },
    ...
    armorMods: armor(
      ['hands-on', 'heavy-ammo-finder', 'stat-weapons'],
      ['heavy-handed', 'focusing-strike', 'stat-health'],
      ['concussive-dampener', 'melee-damage-resistance', 'stat-health'],
      ['recuperation', 'better-already', 'stat-weapons'],
      ['powerful-attraction', 'reaper', 'bomber']
    ),
    baseStats: { weapons: 80, health: 80, class: 20, grenade: 40, super: 30, melee: 30 },
    targetStats: { weapons: 100, health: 100 },
    mechanicIds: ['slow', 'freeze', 'invisibility', 'healing', 'damage-resistance', 'transcendence'],
    championCoverage: { unstoppable: ['冻结后粉碎'], barrier: [], overload: ['闪身施加减速'] },
    rotation: [
  ...
  },
  {
    ...
    abilities: {
      ...
      facetIds: ['facet-protection', 'facet-purpose', 'facet-balance', 'facet-hope']
    },
    ...
    armorMods: armor(
      ['ashes-to-assets', 'heavy-ammo-finder', 'stat-grenade'],
      ['firepower', 'impact-induction', 'stat-grenade'],
      ['concussive-dampener', 'sniper-damage-resistance', 'stat-health'],
      ['recuperation', 'better-already', 'stat-health'],
      ['powerful-attraction', 'reaper', 'bomber']
    ),
    baseStats: { weapons: 20, health: 80, class: 50, grenade: 80, super: 30, melee: 30 },
    targetStats: { health: 100, grenade: 100 },
    mechanicIds: ['amplified', 'slow', 'freeze', 'devour', 'healing', 'transcendence'],
    championCoverage: { unstoppable: ['冻结后粉碎'], barrier: [], overload: ['冰凝炮台持续减速'] },
    rotation: [
  ...
  },
  {
    ...
    armorMods: armor(
      ['heavy-ammo-finder', 'heavy-ammo-scout', 'stat-health'],
      ['focusing-strike', 'stat-grenade'],
      ['concussive-dampener', 'sniper-damage-resistance', 'stat-health'],
      ['solar-weapon-surge', 'solar-weapon-surge', 'recuperation'],
      ['powerful-attraction', 'reaper', 'bomber']
    ),
    baseStats: { weapons: 50, health: 80, class: 30, grenade: 70, super: 50, melee: 30 },
    targetStats: { health: 100, grenade: 80 },
  ...
  },
  {
    ...
    abilities: {
      ...
      facetIds: ['facet-purpose', 'facet-protection', 'facet-balance', 'facet-dawn']
    },
    ...
    armorMods: armor(
      ['heavy-ammo-finder', 'heavy-ammo-scout', 'stat-grenade'],
      ['impact-induction', 'focusing-strike', 'stat-grenade'],
      ['concussive-dampener', 'sniper-damage-resistance', 'stat-health'],
      ['recuperation', 'solar-weapon-surge', 'stat-health'],
      ['powerful-attraction', 'reaper', 'bomber']
    ),
    baseStats: { weapons: 20, health: 80, class: 50, grenade: 80, super: 30, melee: 30 },
    targetStats: { health: 100, grenade: 100 },
  ...
  },
  {
    ...
    armorMods: armor(
      ...
      ['firepower', 'impact-induction', 'stat-grenade'],
      ['concussive-dampener', 'sniper-damage-resistance', 'stat-health'],
      ['solar-weapon-surge', 'recuperation', 'stat-grenade'],
  ...
  ,{
    ...
    armorMods: armor(['heavy-ammo-finder','stat-weapons'],['firepower','focusing-strike','stat-grenade'],['concussive-dampener','sniper-damage-resistance','stat-health'],['recuperation','void-weapon-surge','stat-weapons'],['reaper','powerful-attraction','stat-health']),
    baseStats:{weapons:80,health:80,class:20,grenade:70,super:30,melee:30},targetStats:{weapons:100,health:100,grenade:80},
  ...
  ,{
    ...
    weapons:[weapon('the-call',['Lead from Gold','Slice'],'割裂危险目标并保持特殊弹药'),weapon('sunshot',[],'安全清怪与能量球生成链'),weapon('pro-memoria',['Reconstruction','Target Lock'],'中长距离重弹和清怪')],armorMods:armor(['heavy-ammo-finder','heavy-ammo-scout','stat-health'],['firepower','impact-induction','stat-grenade'],['concussive-dampener','sniper-damage-resistance','stat-health'],['recuperation','better-already','stat-grenade'],['reaper','powerful-attraction','bomber']),
    baseStats:{weapons:20,health:80,class:40,grenade:80,super:30,melee:40},targetStats:{health:100,grenade:100},mechanicIds:['suspend','woven-mail','sever','healing'],championCoverage:{barrier:[],overload:[],unstoppable:['德兰格之鞭与束缚手雷悬浮']},
  ...
  ,{
    ...
    weapons:[weapon('the-call',['Lead from Gold','Slice'],'特殊弹药与割裂'),weapon('no-hesitation',['Physic','Circle of Life'],'直接治疗队友与支援增益'),weapon('apex-predator',['Reconstruction','Bait and Switch'],'机制与首领阶段重弹')],armorMods:armor(['heavy-ammo-finder','heavy-ammo-scout','stat-grenade'],['impact-induction','stat-grenade'],['concussive-dampener','sniper-damage-resistance','stat-health'],['recuperation','solar-weapon-surge','stat-health'],['powerful-attraction','reaper','bomber']),
    baseStats:{weapons:20,health:80,class:50,grenade:80,super:30,melee:30},targetStats:{health:100,grenade:100},mechanicIds:['healing','cure','restoration','radiant','sever'],championCoverage:{barrier:['光耀武器可穿透屏障冠军护盾'],overload:[],unstoppable:['点燃可眩晕不可阻挡；需要稳定累积灼烧']},
